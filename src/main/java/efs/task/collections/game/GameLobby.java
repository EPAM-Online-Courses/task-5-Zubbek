...
public class GameLobby {
    ...
    public void disableDLC() {
        ...
        for (Town dlcTown : dlcTowns) {
            List<Hero> townHeroes = playableTownsWithHeroesList.get(dlcTown);
            if (townHeroes != null) {
                townHeroes.clear();
            }
    ...
    public Map<Town, List<Hero>> mapHeroesToStartingTowns(List<Town> availableTowns, Set<Hero> availableHeroes) {
        Map<Town, List<Hero>> heroesByTownMap = new TreeMap<>(Comparator.comparing(Town::getTownName));
        ...
        for (Town town : availableTowns) {
            ...
            heroesByTownMap.put(town, heroesList);
        }

        return heroesByTownMap;
    }
